...
export default function EditArticlePage() {
  ...
  async function fetchArticle() {
    ...
    const { data, error } = await supabase
      .from('articles')
      .select('title, slug, excerpt, content, thumbnail_url, category, is_published')
      .eq('id', articleId)
      .single()
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!formData.title.trim()) { alert('タイトルを入力してください'); return }
    if (!formData.slug.trim()) { alert('スラッグを入力してください'); return }
    if (!formData.content.trim() || formData.content === '<p></p>') { alert('本文を入力してください'); return }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/admin/articles" className="p-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
          <ArrowLeft size={20} />
        </Link>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-lg shadow-sm p-6 space-y-6">
          ...
          {/* Slug */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">スラッグ <span className="text-red-500">*</span></label>
            <div className="flex items-center">
              <span className="px-3 py-2 bg-gray-100 border border-r-0 border-gray-300 rounded-l-lg text-gray-500 text-sm">/articles/</span>
              <input
                type="text"
                value={formData.slug}
          ...
          <div>
            ...
            {formData.thumbnail_url ? (
              <div className="relative inline-block">
                <img src={formData.thumbnail_url} alt="サムネイル" className="w-64 h-36 object-cover rounded-lg border border-gray-200" />
                <button
                  type="button"
        ...
        <div className="flex justify-end gap-4">
          <Link href="/admin/articles" className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
            キャンセル
          </Link>
          <button
            type="submit"
